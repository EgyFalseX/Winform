    ...
    Sub main()
        ...
        If FunLib.GetRegValue(Str, FunctionLib.DataCenterServerLib.RegKeyValues.SQLString) = True Then
            ...
            If ErrorProvider <> Nothing Then
                MsgBox_("áã äÊãßä ãä ÊÔÛíá ÇáÈÑäÇãÌ ' ÍØÇÁ Ýí ÇáÎíÇÑÇÊ '", 20, 23, False, StartUpFrm)
                If MsgBox("ãä Çáããßä Çä íßæä ÎØÇÁ Ýí ÌãáÉ ÇáÇÊÕÇá" & vbNewLine & "(ÇáÛÇÁ ÇáÇÊÕÇá)åá ÇäÊ ãÊÃßÏ ?", MsgBoxStyle.YesNo, "Warning") = MsgBoxResult.Yes Then
                    FunLib.ResetConnection()
                End If
                Process.GetCurrentProcess.Kill()
            End If
            If ProjectsPath = "" Or RelatedFiles = "" Or REQPort = "" Or ServerIP = "" Or DeletedFilesPath = "" Then
                MsgBox_("áã íÊã ÊÝÚíá ÇáÎíÇÑÇÊ Ýí ÇáÎÇÏã", 0, 23, False, New Form)
                Process.GetCurrentProcess.Kill()
            End If
            FrmLogin.ShowDialog()
        Else
