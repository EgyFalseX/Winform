    ...
    public partial class SQLConnectionUC : UserControl
    {
        ...
        private void BtnConnect_Click(object sender, EventArgs e)
        {
            if (CBSqlAuth.SelectedIndex == 0)
                FullConString = FXFW.SqlDB.ServerTest(txtserver.Text.Trim(), TxtID.Text.Trim(), TxtPW.Text.Trim());
            else if (CBSqlAuth.SelectedIndex == 1)
                FullConString = FXFW.SqlDB.ServerTest(txtserver.Text.Trim());
        ...
        private void RBAttached_CheckedChanged(object sender, EventArgs e)
        {
            if (RBAttached.Checked)
            {
                ...
                DataTable AllDB = FXFW.SqlDB.GetAllDatabase(FullConString);
        ...
        private void btnstart_Click(object sender, EventArgs e)
        {
            ...
            if (RBNotAttached.Checked)
            {
                ...
                {
                    OkiDoki = FXFW.SqlDB.AttachDB(DatabaseName, DBPath, DBLogPath, FullConString);
                    if (OkiDoki)
                ...
                else if (DBPath != string.Empty && DBLogPath == string.Empty)
                {
                    ...
                    OkiDoki = FXFW.SqlDB.AttachDB(DatabaseName, DBPath, FullConString);
                    if (OkiDoki)
                        FullConString += ";Initial Catalog=" + DatabaseName; //Set ConSTR in SQLCON and Registry
                    else
                        return;
            ...
            if (CBUseExpress.Checked)
            {
                ...
                FXFW.SqlDB.ReportURL = String.Format("http://{0}:{1}/{2}", servername, TxtReportSrvPort.Text.Trim(), TxtReportSrvName.Text.Trim());
            }
            else
                FXFW.SqlDB.ReportURL = String.Format("http://{0}:{1}/{2}", txtserver.Text.Trim(), TxtReportSrvPort.Text.Trim(), TxtReportSrvName.Text.Trim());
            char[] x = TxtReportPath.Text.Trim().ToCharArray();
            if (x[0].Equals('/') == false)
                TxtReportPath.Text = "/" + TxtReportPath.Text.Trim();
            if (x[x.Length - 1].Equals('/') == false)
                TxtReportPath.Text = TxtReportPath.Text.Trim() + "/";
            FXFW.SqlDB.ReportPath = TxtReportPath.Text;
            FXFW.SqlDB.SqlConStr = FullConString;// Activate SqlConstr
            FXFW.SqlDB.SaveSQLDBPath();
            Application.Restart();
        }
