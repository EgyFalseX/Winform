...
namespace Training_Center
{
    public partial class TBLAccountsFrm : XtraForm
    {
        ...
        private void LoadNodes(string ParentID, TreeListNode Nodi, bool LoadChildNodes)
        {
            ...
            NodesTbl = MyCL.LoadDataTable("SELECT AccountId, AccountTreeId, AccountDes, (SELECT KhtamiaccName FROM CDKHTAMIACOUNT WHERE (KhtamiaccID = TBL_Accountes.KhtamiaccID)) AS AsKhtamiaccName, (SELECT AccNatueName FROM CDAccountNature WHERE (AccNatueID = TBL_Accountes.AccNatueID)) AS AccNatueName, AccountBudget, AccountSort FROM TBL_Accountes WHERE (PairantAccount = " + ParentID + ") ORDER BY AccountSort");
        ...
        private void FillDefaultData()
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("", MyCL.SqlConStr);
            try
            {
