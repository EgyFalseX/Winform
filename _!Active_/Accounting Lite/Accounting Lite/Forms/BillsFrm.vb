Public Class BillsFrm
    ...
    Private Sub CBTypesName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CBTypesName.SelectedIndexChanged
        If CBTypesName.SelectedIndex > -1 Then
            TxtNotes.Text = CBTypesName.DataSource.rows(CBTypesName.SelectedIndex).item(2).ToString
            CBIncomings.DataSource = CreateOLETable("SELECT Incoming.IncomingID, Incoming.InCount - (select IIF (IsNull(Sum(ConCount)),0,Sum(ConCount)) from ContainZ where (TypeID = " & CBTypesName.DataSource.rows(CBTypesName.SelectedIndex).item(0).ToString & ") and (Incoming.IncomingID = ContainZ.IncomingID) )   AS IncomingCount, Incoming.InValue FROM (Incoming LEFT OUTER JOIN ContainZ ON Incoming.IncomingID = ContainZ.IncomingID AND Incoming.IncomingID = ContainZ.IncomingID) WHERE (Incoming.TypeID = " & CBTypesName.DataSource.rows(CBTypesName.SelectedIndex).item(0).ToString & ") group by Incoming.IncomingID, InCount,InValue")
            CBIncomings.ValueMember = "IncomingID" : CBIncomings.DisplayMember = "IncomingID"
            If CBIncomings.Items.Count > 0 Then
                CBIncomings.Enabled = True : CBIncomings.SelectedIndex = 0
            End If
        Else
            BtnAdd.Enabled = False
        End If
        BtnAdd.Enabled = False
    End Sub
    Private Sub CBIncomings_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CBIncomings.SelectedIndexChanged
        If CBIncomings.SelectedIndex > -1 Then
            TxtCost.Text = Val(CBIncomings.DataSource.rows(CBIncomings.SelectedIndex).item(2).ToString)
            TxtContainCount.Text = Val(CBIncomings.DataSource.rows(CBIncomings.SelectedIndex).item(1).ToString)
            TxtCount.Text = "0"
            BtnAdd.Enabled = True
        End If
    End Sub
    Private Sub BtnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnAdd.Click
        If Val(TxtCount.Text.Trim) <> 0 And Val(TxtCount.Text.Trim) <= Val(TxtContainCount.Text.Trim) Then
            ...
            If Insertion_Updateing_Deleting("INSERT INTO ContainZ (BillID, TypeID, ConCount, ConCost, IncomingID) VALUES ('" & TxtBillNumber.Text.Trim & "'," & CBTypesName.SelectedValue.ToString & ",'" & Val(TxtCount.Text.Trim) & "','" & Val(TxtCost.Text.Trim) & "'," & CBIncomings.SelectedValue.ToString & ")", EffectedID) = Nothing Then
                Dim lvi As New ListViewItem
                ...
                CBIncomings.DataSource = Nothing
